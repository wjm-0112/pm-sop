...
function EditVersionPage() {
  ...
  const submit = async (v: VersionFormValues) => {
    ...
    router.push(`/planning/versions/detail?id=${version.id}`);
  };
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      <VersionForm
        initial={initial}
        onSubmit={submit}
        onCancel={() => router.push(`/planning/versions/detail?id=${version.id}`)}
      />
    </div>
  );
}
...
export default function EditVersionPageWrap() {
  return (
